...
{
    printf("\n=== Configurar conexao ===\n");
    printf("Digite o tipo de entrada (1 USB, 2 RS232, 3 TCP/IP, 4 Bluetooth, 5 Acoplada):\n");
    scanf("%d", &g_tipo);

    printf("Digite o modelo (ex: i9):\n");
    scanf("%s", g_modelo);

    printf("Digite o tipo de conexao (ex: USB, COM2, IP):\n");
    scanf("%s", g_conexao);

    printf("Digite o parametro (baudrate/portaTCP, 0 padrao):\n");
    scanf("%d", &g_parametro);
...
static void abrirConexao(void)
{
    if (!g_conectada)
    {
        ...
        if (ret == 0) {
            ...
            printf("Falha na conexao. Retorno %d\n", ret);
        }
    }
    else {
...
if (ret == 0) {
	...
	printf("Impressao, Avanco de Papel e Corte realizados com sucesso!");
} else {
	printf("Impressao e Avanco de Papel OK, mas falha no Corte. (Retorno: %d)",ret);
}
}
...
//{
//    int tipo = 8;
//    const char *dados = "A012345678912";
//    int altura = 100;
//    int largura = 2;
//    int HRI = 3;
...
static void abrirGavetaElgin(){
 ...
 static void abrirGaveta (){
        
    int pino = 1;
    int tempoInicio = 5;
    int tempoFim = 10;
    
    int retorno = AbreGaveta(pino, tempoInicio, tempoFim);
...
static void emitirSinalSonoro(void)
{
    ...
    int ret = SinalSonoro(qtd, tempoInicio, tempoFim);

    if (ret == 0)
        printf("Sinal sonoro emitido.\n");
    else
        printf("Erro ao emitir som.\n");
}
